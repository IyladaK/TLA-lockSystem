    ...
    if ~InLock /\ req.side = "west" then
        ...
        if ~(waterLevel = "low") then
            ...
            if ~BothDoorsClosed then
                lock.command = [command |-> "change_door", open |-> FALSE, side |-> OpenDoor]
                await lockCommand.command = "finished";
            lock.command = [command |-> "change_valve", open |-> TRUE, side |-> "low"]
            await lockCommand.command = "finished";
...
\* Last modified Tue Oct 07 11:23:39 CEST 2025 by iyladakeekarjai
